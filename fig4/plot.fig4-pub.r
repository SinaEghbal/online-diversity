...
for (ds in datasets) {
  ...
  plot(NA, xlab = 'Age (years)', ylab = 'Proportion of domains active', ylim = c (0, 1), xlim=c(0,max(j)/12))
  for (i in seq_len(nrow(survival_clusters2))) {
    ...
    x <- (j-start)/12
    points (x, survival_clusters2[i, ], col = colors[i], type = 'l')
    text((end-start)/12, survival_clusters2[i, end], rownames(survival_clusters2)[i], col = colors[i], pos=1, cex=0.8)
  }
  rm (list = data)
  dev.off ()
}